...
function loadApplication(reload) {
	reload = reload ? reload : (initialLoadData.loginErrorMessage ? (initialLoadData.loginErrorMessage != "" ? true : false) : false);
	...
	if (status == "error") {
		if (errorCode == "1" && errorMessage == "") {
			...
			if (errorCode == "2"){
				...
				// are there more error codes that have the same behavior ??? -- yes, so moved it down for all 
...
function processServerErrors(params) {
	...
	if (params.server_errors["update-error"]){
		...
		try {
			/* 
			   During normal processing when the update is sent to the backend Rho removes them from changed values so they will not be sent again.
			   If you look at the ruby code in rhom_object_factory.rb they are processing each attribute checking to see if it exists in 
			   changed_values already. If it does they leave it, if not they reinsert it.  
			...
			*/
			...
			for (var property2 in update_error) {
				if (update_error.hasOwnProperty(property2)){
					...
					for (var attrib in update_error[property2].attributes){
						if (update_error[property2].attributes.hasOwnProperty(attrib)){
							if (attrib != "object"){
								attribValue = update_error[property2].attributes[attrib];
								
								sqlExpression = [];
								sqlExpression[0] = property2;
								sqlExpression[1] = params.source_id;
								sqlExpression[2] = attrib;
								sqlExpression[3] = 0;
								sql = "select * ";
								sql += "from changed_values ";
								sql += "where object = ? ";
								sql += "and source_id = ? ";
								sql += "and attrib = ? ";
								sql += "and sent = ?";
								...
								if (resultArray.length > 0){
									...
									sqlExpression = [];
									sqlExpression[0] = params.source_id;
									sqlExpression[1] = attrib;
									sqlExpression[2] = property2;
									sqlExpression[3] = attribValue;
									sqlExpression[4] = "";
									sqlExpression[5] = "update";
									sqlExpression[6] = 0;
									sql = "insert into changed_values ";
...
function logErrors(jsonObj, mode){
	Rho.Log.info("Start: logErrors(" + mode + ")", "inMotion");
	var j;
	if (mode == "checkout"){
		for (j = 0; j < jsonObj.checkout.errorList.length; j++){
			Rho.Log.info(jsonObj.checkout.errorList[j].errorMessage, "inMotion");
		}
	}
	else if (mode == "capture signature"){
		for (j = 0; j < jsonObj.captureSignature.errorList.length; j++){
			Rho.Log.info(jsonObj.captureSignature.errorList[j].errorMessage, "inMotion");
		}
	}
	else if (mode == "close trip"){
		for (j = 0; j < jsonObj.closeTrip.errorList.length; j++){
			Rho.Log.info(jsonObj.closeTrip.errorList[j].errorMessage, "inMotion");
		}
	}
	for (j = 0; j < jsonObj.syncErrors.errorList.length; j++){
		Rho.Log.info(jsonObj.syncErrors.errorList[j].errorMessage, "inMotion");
	}
	Rho.Log.info("End: logErrors", "inMotion");
}