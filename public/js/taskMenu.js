	...
	$.get("/public/templates/addItemScan.html", function(data){
		modal.open({
			...
			if(!prefix.match(text)){
			   errMsgs.push("Prefix can only contain A-Z, a-z, 0-9, or space characters. \n");
			}
			var numbers = /^[0-9]+$/;
			...
			if (validateTaskHeaderContainer()){
				...
				errMsgs.push("Task header is missing");
			}
...
function assignTaskPatient(id){
	...
	if(validateTaskHeaderContainer()){
		...
		alert("Unable to display patient list because task header information is missing");
	}
...
function saveEditOrderItem(mode){
	...
	if (!validateTaskHeaderContainer()){
		errMsgs.push("Task header information is missing. \n");
	}
	if (!validateTripContainer()){
		errMsgs.push("Trip information is missing. \n");
	}
...
function editOrderItem(prefix, department, purchaseOrder){
	...
	if (validateTaskHeaderContainer()){
		...
		alert("Unable to edit order item because task header information is missing.");
	}
...
function editTask(){
	...
	$.get("/public/templates/editTask.html", function(data){
		...
		alert("Unable to edit task because task information is missing.");
	}
...
function editTaskPatient(){
	...
	if (validateTaskHeaderContainer()){
		...
		alert("Unable to edit the patient on task because task header information is missing.");
	}
...
function refuseSelectedItems(){
	...
	if(validateTaskHeaderContainer()){
		...
		alert("Unable to display refused item dialog because task information is missing.");
	}
	Rho.Log.info("End: refuseSelectedItems", "inMotion");
}

function removeDamagedItemByTask(localTransferNumber){
	Rho.Log.info("Start: removeDamagedItemByTask(" + localTransferNumber + ")", "inMotion");
	...
	sql += "where localTransferNumber = ? ";
	var damagedItems = userDatabase.executeSql(sql, sqlExpression);

	for (var j = 0; j < damagedItems.length; j++){
		removeDamagedItemByItem(localTransferNumber, damagedItems[j].prefix, damagedItems[j].unit);
	}
	Rho.Log.info("End: removeDamagedItemByTask", "inMotion");
}

function removePreferredAccessoryByTask(hasUnitAccessoryTracking, localTransferNumber){
	Rho.Log.info("Start: removePreferredAccessoryByTask(" + localTransferNumber + ")", "inMotion");
	...
	if (hasUnitAccessoryTracking == 0){
		sql = "select prefix, count(*) as unitCount ";
		sql += "from TransferDetail ";
		sql += "where localTransferNumber = ? ";
		sql += "and prefix in ( ";
		sql += "select prefix ";
		...
		for (j = 0; j < accessoriesList.length; j++){
			var unitCount = accessoriesList[j].unitCount;
			for (k = 0; k < unitCount; k++){
				removePreferredAccessoryByItem(hasUnitAccessoryTracking, localTransferNumber, accessoriesList[j].prefix, unitCount - k, 0);
			}
		}
	}
	else {
		sql = "select localTransferNumber, "; 
		sql += "prefix, ";
		sql += "unit ";
		sql += "from TransferredUnitAccessories ";
		sql += "where localTransferNumber = ? ";
		sql += "group by localTransferNumber, prefix, unit ";
		accessoriesList = userDatabase.executeSql(sql, sqlExpression);

		for (j = 0; j < accessoriesList.length; j++){
			removePreferredAccessoryByItem(hasUnitAccessoryTracking, localTransferNumber, accessoriesList[j].prefix, 0, accessoriesList[j].unit);
		}
	}
	Rho.Log.info("End: removePreferredAccessoryByTask", "inMotion");
}

function removeRefusalByTask(localTransferNumber, currentEmployeeId){
	Rho.Log.info("Start: removeRefusalByTask(" + localTransferNumber + ")", "inMotion");
	
	var sqlExpression = [];
	sqlExpression[0] = localTransferNumber;
	
	var sql = "SELECT prefix, unit ";
	sql += "FROM TransferDetail ";
	sql += "where localTransferNumber = ? ";
	sql += "and refusedFlag = 'Y' ";
	var refusedItems = userDatabase.executeSql(sql, sqlExpression);

	for (var j=0; j < refusedItems.length; j++){
		removeRefusalByItem(localTransferNumber, refusedItems[j].prefix, refusedItems[j].unit, currentEmployeeId);
	}
	Rho.Log.info("End: removeRefusalByTask", "inMotion");
}

function removeSwapOutByTask(localTransferNumber){
	Rho.Log.info("Start: removeSwapOutByTask(" + localTransferNumber + ")", "inMotion");
	...
	for (var j = 0; j < swapoutList.length; j++){
		removeSwapOutByItem(localTransferNumber, swapoutList[j].pickupPrefix, swapoutList[j].pickupUnit);
	}
	Rho.Log.info("End: removeSwapOutByTask", "inMotion");
}

function removeTransferDetailByTask(localTransferNumber){
	Rho.Log.info("Start: removeTransferDetailByTask(" + localTransferNumber + ")", "inMotion");
	...
	for (var i = 0; i < transferDetailList.length; i++){
		removeTransferDetailByItem(localTransferNumber, transferDetailList[i].prefix, transferDetailList[i].unit);
	}
	Rho.Log.info("End: removeTransferDetailByTask", "inMotion");
}

function removeTripDetailByTask(localTransferNumber){
	Rho.Log.info("Start: removeTripDetailByTask(" + localTransferNumber + ")", "inMotion");
	...
	Rho.Log.info("End: removeTripDetailByTask", "inMotion");
}

function resetTransferHeaderByTask(localTransferNumber, currentEmployeeId){
	Rho.Log.info("Start: resetTransferHeaderByTask(" + localTransferNumber + ", " + currentEmployeeId + ")", "inMotion");
	...
	Rho.Log.info("End: resetTransferHeaderByTask", "inMotion");
	...
	if (msg === true){
		if (validateTaskHeaderContainer() && validateTripContainer()){
			...
			try	{
				...
				//Remove Refusals
				removeRefusalByTask(localTransferNumber, currentEmployeeId);
				
				//find all accessories
				removePreferredAccessoryByTask(hasUnitAccessoryTracking, localTransferNumber);
				
				//Remove damaged item records
				removeDamagedItemByTask(localTransferNumber);
				
				//remove swap outs
				removeSwapOutByTask(localTransferNumber);
				
				//remove trip detail records for this transfer
				removeTripDetailByTask(localTransferNumber);
	
				//remove transfer detail records
				removeTransferDetailByTask(localTransferNumber);
	
				//reset transfer header
				resetTransferHeaderByTask(localTransferNumber, currentEmployeeId);
			...
			alert("Unable to remove task because task header information is missing");
		}
	}
	...
	if (msg === true){
		...
		if(validateTaskHeaderContainer() && validateTripContainer()){
			try {
				...
				Rho.Log.info("Error: removeTaskPatient rollback=" + e, "inMotion");
				userDatabase.rollbackTransaction();
				alert("Unable to remove patient from task (" + e + ")");
			}
		}
		else {
			alert("Unable to remove patient from task because task header or trip information was not available.");
		}
	}
		...
		$(".patientList[src='images/uhs/radiobutton-checked.png']").each(function(){
			if (validateTaskHeaderContainer() && validateTripContainer()){
				...
				try {
					...
					Rho.Log.info("Error: saveAssignTaskPatient rollback=" + e, "inMotion");
					userDatabase.rollbackTransaction();
					alert("Unable to assign patient to task (" + e + ")");
				}
			}
			else {
				alert("Unable to assign patient to task because transfer information missing");
				return false;
			}
		});
	}
...
function saveEditTaskPatient(){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
		...
		try {
			...
			Rho.Log.info("Error: saveEditTaskPatient rollback=" + e, "inMotion");
			userDatabase.rollbackTransaction();
			alert("Unable to edit assigned patient (" + e + ")");
		}
	}
	else {
		alert("Unable to edit assigned patient on task because transfer information missing");
	}
...
function saveRefusedItems(){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
		...
		alert("Unable to save refused items because task header or trip information is missing");
	}
...
function viewTransferOrder(id){
	...
	if (validateTaskHeaderContainer()){
		...
		alert("Unable to view transfer order because task header information is missing.");
	}
