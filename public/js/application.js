		...
		if (mode > -1) {
			try {
				...
				if (Rho.System.isMotorolaDevice) {
					...
					$("#frameListWrapper").append("<button class='fab' id='cameraFab'><img src='images/uhs/ic_photo_camera_white_24dp.png'></button>");
...
function scannerCallbackMode1(e){
	...
	if (data) {
		...
		if (data.length != 7){
			Rho.Log.info("Error: Data length of scan not equal to 7", "inMotion");
			scanContinue = false;
		}
		var text = /^[a-zA-Z0-9 ]+$/;
		if(!data.substr(0,3).match(text)){
			Rho.Log.info("Error: Prefix can only contain A-Z, a-z, 0-9, or space characters", "inMotion");
			scanContinue = false;
		}
		//check last four digits are a number between 0 and 9999
		var numbers = /^[0-9]+$/;
		if(!data.substr(3,4).match(numbers)){
			Rho.Log.info("Error: Last 4 digits of scan are not numbers", "inMotion");
		   	scanContinue = false;
		}
		else {
			if (parseInt(data.substr(3,4), 10) <= 0 || parseInt(data.substr(3,4), 10) > 9999){
				scanContinue = false;
				Rho.Log.info("Error: Last 4 digits of scan are not between range 0 and 9999", "inMotion");
			}
		}
		
		//check if prefix unit is already on trip
		var sqlExpression = [];
		sqlExpression[0] = data.substr(0,3);
		sqlExpression[1] = data.substr(3,4);
		...
		if (transferDetailArray.length > 0){
			Rho.Log.info("Error: " + data + " has already been scanned on trip", "inMotion");
			scanContinue = false;
		}
		
		if (!validateTaskHeaderContainer()){
			Rho.Log.info("Error: task header information is missing", "inMotion");
			scanContinue = false;
		}
		if (!validateTripContainer()){
			Rho.Log.info("Error: trip information is missing", "inMotion");
			scanContinue = false;
		}
...
function scannerCallbackMode3(e){
	...
	if (data) {
		var scanContinue = true;
		if (data.length != 17){
			Rho.Log.info("Error: Data length of scan not equal to 17", "inMotion");
			scanContinue = false;
		}
		if (scanContinue){
...
function applicationSendLog(callBack) {
	if (!Rho.System.isRhoSimulator){
		...
		if (host == "rho4prod" || shortHost == "rho4prod") {
			Rho.Log.destinationURI = "http://www2.uhs.com:17000/InMotionLogRestApi/jaxrs/log";
		}
		else {
...
function validateTaskHeaderContainer(){
	Rho.Log.info("Start: validateTaskHeaderContainer()", "inMotion");
	var containerContinue = true;
	//check that data from taskHeaderContainer is not empty or missing
	var taskHeaderContainer = null;
	taskHeaderContainer = document.getElementById("taskHeaderContainer");
	var transferNumber = null;
	var localTransferNumber = null;
	var transferType = null;
	var hasUnitAccessoryTracking = null;
	var accountId = null;
	var scanLevel = null;
	var transferDate = null;
	var swapOutNumber = null;
	var localSwapOutNumber = null;
	...
	if (taskHeaderContainer){
		...
		Rho.Log.info("Error: taskHeaderContainer is missing", "inMotion");
		containerContinue = false;
	}
	Rho.Log.info("End: validateTaskHeaderContainer", "inMotion");
...
function validateTripContainer(){
	...
	var currentEmployeeInitials = null;
	var currentEmployeeId = null;
	var tripId = null;
	var localTripId = null;
	if (tripContainer){
