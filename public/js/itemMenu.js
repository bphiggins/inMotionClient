function addScan(id) {
	...
	if (validateTaskHeaderContainer()){
		...
		alert("Unable to add scan off vehicle because task header information is missing.");
	}
...
function clearRefusal(id){
	...
	if (msg === true){
		if (validateTaskHeaderContainer() && validateTripContainer()){
			userDatabase.startTransaction();
			try {
				var localTransferNumber = $("#taskHeaderContainer").attr("data-localTransferNumber");
				var transferType = $("#taskHeaderContainer").attr("data-transferType");
				...
				removeRefusalByItem(localTransferNumber, prefix, unit, currentEmployeeId);
	
				//update transferDetail record to remove refusal fields
				var sqlExpression = [];
				sqlExpression[0] = localTransferNumber;
				sqlExpression[1] = prefix;
				sqlExpression[2] = unit;
			...
			Rho.Log.info("Error: unable to clear refusal because task header information is missing.", "inMotion");
			alert("unable to clear refusal because task header information is missing.");
		}
	}
...
function damagedItem(id) {
	...
	if (validateTaskHeaderContainer()){
		...
		Rho.Log.info("Error: unable to display damaged item because task header information is missing.", "inMotion");
		alert("Unable to display damaged item because task header information is missing.");
	}
...
function editItem(id) {
	...
	if (validateTaskHeaderContainer()){
		...
		Rho.Log.info("Error: unable to edit item because task header information is missing.", "inMotion");
		alert("Unable to edit item because task header information is missing.");
	}
...
function editItemPatient(id) {
	...
	if (validateTaskHeaderContainer()){
		...
		Rho.Log.info("Error: unable to edit item patient because task header information is missing.", "inMotion");
		alert("Unable to edit item patient because task header information is missing.");
	}
...
function editPreferredAccessory(id) {
	...
	if (validateTaskHeaderContainer()){
		...
		Rho.Log.info("Error: unable to edit preferred accessory because task header information is missing", "inMotion");
		alert("Unable to edit preferred accessory because task header information is missing.");
	}
...
function refuseItem(id){
	...
	if (validateTaskHeaderContainer()){
		...
		Rho.Log.info("Error: unable to display refuse item dialog because task header information is missing", "inMotion");
		alert("Unable to display refuse item dialog because task header information is missing.");
	}
	Rho.Log.info("End: refuseItem", "inMotion");
}

function removeDamagedItemByItem(localTransferNumber, prefix, unit){
	Rho.Log.info("Start: removeDamagedItemByItem(" + localTransferNumber + ", " + prefix + ", " + unit + ")", "inMotion");
	...
	var localDamagedItemId, damagedItemDetailList, damagedItemInstance, damagedItemDetailInstance;

	for (var i = 0; i < damagedItemList.length; i++){
		...
		damagedItemDetailList = userDatabase.executeSql(sql, sqlExpression);

		for (j = 0; j < damagedItemDetailList.length; j++){
			damagedItemDetailInstance = damagedItemDetailModel.make({
	...
	Rho.Log.info("End: removeRefusalByItem", "inMotion");
}
...
function removeItemPatient(id){
	...
	if (msg === true){
		...
		if (validateTaskHeaderContainer()){
			...
			Rho.Log.info("Error: unable to remove patient from item because task header information is missing", "inMotion");
			alert("Unable to remobe patient from item because task header information is missing.");
		}
	}
	Rho.Log.info("End: removeItemPatient", "inMotion");
}

function removePreferredAccessoryByItem(hasUnitAccessoryTracking, localTransferNumber, prefix, unitCount, unit){
	Rho.Log.info("Start: removePreferredAccessoryByItem(" + hasUnitAccessoryTracking + ", " + localTransferNumber + ", " + prefix + ", " + unit + ")", "inMotion");
	...
	if (hasUnitAccessoryTracking == 0){
		if (unitCount == -1){
			...
			sql += "where localTransferNumber = ? ";
			sql += "and prefix= ? ";
			var result = userDatabase.executeSql(sql, sqlExpression);
			if (result.length > 0){
				unitCount = result[0].unitCount;
			}
		}
		...
		var accessoryPerUnitCount;
		var adjAccessoryQuantity;

		for (i = 0; i < accessoriesList.length; i++){
			accessoryPerUnitCount = accessoriesList[i].quantity / unitCount;
			adjAccessoryQuantity = accessoryPerUnitCount * (unitCount - 1);
	...
	Rho.Log.info("End: removePreferredAccessoryByItem", "inMotion");
}

function removeRefusalByItem(localTransferNumber, prefix, unit, currentEmployeeId){
	Rho.Log.info("Start: removeRefusalByItem(" + prefix + ", " + unit + ")", "inMotion");
	...
	var refusalPickupInstance, refusalPickupDetailInstance, commentInstance, tripDetailInstance;
	
	for (i = 0; i < refusalList.length; i++){
		refusalPickupInstance = transferHeaderModel.make({
		...
		refusalPickupDetailInstance = transferDetailModel.make({
			"cstat": refusalList[i].cstat,
			"department": refusalList[i].department,
...
function removeSwapOutByItem(localTransferNumber, prefix, unit){
	Rho.Log.info("Start: removeSwapOutByItem()", "inMotion");
	...
	for (var i = 0; i < swapoutList.length; i++){
	...
	Rho.Log.info("End: removeSwapOutByItem", "inMotion");
}

function removeTransferDetailByItem(localTransferNumber, prefix, unit){
	Rho.Log.info("Start: removeTransferDetailByItem()", "inMotion");
	...
	Rho.Log.info("End: removeTransferDetailByItem", "inMotion");
}
...
function removeScan(id){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
			...
			*/
			...
			try {
				...
				if (scanLevel == "0"){
					...
					removePreferredAccessoryByItem(hasUnitAccessoryTracking, localTransferNumber, prefix, -1, unit);
	
					//remove any swapouts
					removeSwapOutByItem(localTransferNumber, prefix, unit);
					
					//remove any Damaged Items and Damaged Item Details
					removeDamagedItemByItem(localTransferNumber, prefix, unit);
					
					//remove any reufusal pickup transfers, transferdetails, and comments. look for closed pickups with this prefix unit
					removeRefusalByItem(localTransferNumber, prefix, unit, currentEmployeeId);
			
					//reset transferHeader record if all items have been removed
					sqlExpression = [];
					sqlExpression[0] = localTransferNumber;
					...
					if (transferDetailCountArray.length > 0){
						if (transferDetailCountArray[0].transferDetailCount == 1){
							//retrieve transferHeader
							resetTransferHeaderByTask(localTransferNumber);
						}
					}
	
					//remove transfer detail record
					removeTransferDetailByItem(localTransferNumber, prefix, unit);
		...
		Rho.Log.info("Error: unable to remove scan because task container information is missing", "inMotion");
		alert("Unable to remove scan because task container information is missing.");
	}
	...
	if ($(".patientList[src='images/uhs/radiobutton-checked.png']").length > 0){
		$(".patientList[src='images/uhs/radiobutton-checked.png']").each(function(){
			if (validateTaskHeaderContainer()){
				...
				alert("Unable to assign patient to item because task header information is missing.");
				return false;
			}
		});
	}
...
function saveEditItemPatient(id){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
		...
		alert("Unable to save assigned patient because task header or trip information missing.");
	}
...
function saveFindEquipment(){
	...
	if (validateTripContainer()){
		...
		alert("Unable to save find equipment because task header or trip information is missing");
	}
...
function saveItem(){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
		...
		alert("Unable to save item because task header or trip information missing");
	}
...
function savePreferredAccessory(){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
							...
							for (j = 0; j < retrievedUnitAccessoriesArray.length; j++){
								if ((j + 1) > deliveredCount){
									...
									var surplusRetrievedAccessoriesInstance = surplusRetrievedAccessoriesModel.create({
										...
										"localSurplusId": 0,
										"matchDate": '0001-01-01 00:00:00.0',
										"matchUserId": 0,
		...
		alert("Unable to save preferred accessory because task header or trip information is missing.");
		return false;
	}
...
function swapout(id){
	...
	if (validateTaskHeaderContainer()){
		...
		alert("Unable to display swapout because task header information is missing.");
	}
...
function takeDamagedItemPicture(){
	...
	if(!Rho.System.isEmulator && !Rho.System.isRhoSimulator && Rho.System.hasCamera) {
		...
		if (continueOn) {
			saveDamagedItemPicture({"imageUri": "data:image/png;base64,","status":"ok"});
		}
	}
	else{
...
function saveDamagedItemPicture(params){
	...
	if (validateTaskHeaderContainer() && validateTripContainer()){
		if (params["status"]=="ok") {
			var img = params["imageUri"].replace(/^data:image\/[^;]+;base64,/,'');
			var damagedItemId = $("#damagedItemId").val();
			var localDamagedItemId = $("#localDamagedItemId").val();
			var currentEmployeeId = $("#tripContainer").attr("data-currentEmployeeId");
			...
			alert("Unable to capture picture. " + params["message"]);
		}
	}
	else {
		alert("Unable to save damaged item picture because task header or trip information is missing.");
	}
